<template>
  <div class="results">
    ...
    <ul>
      <li class="movie-item" v-for="(result, index) in results" v-bind:key="index">
        ...
        <div class="ratings">
          ...
          <span class="rating-category well-liked" v-else-if="result.vote_average > 7">A Decent Watch</span>
          <span class="rating-category stinker" v-else>Absolutely Rotten</span>
          <span class="vote-average">{{ result.vote_average }}</span> with <span class="vote-count">{{result.vote_count}}</span> votes 
        </div>
        <p class="overview">
          {{result.overview + " Debbie's personal choices include the following: Deadpool, Spirited Away, Howl's Moving Castle, and Black Panther."}}
        </p>
